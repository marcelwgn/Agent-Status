...
#   msbuild AIStatusTray.slnx /p:Platform=x64 /p:Configuration=Debug
...
$content = $content -replace '\$targetnametoken\$', 'AIStatusTray.CmdPalExtension'
$content = $content -replace '\$targetentrypoint\$', 'AIStatusTray.CmdPalExtension.exe'
...
$existing = Get-AppxPackage -Name "AIStatusTray.CmdPalExtension" -ErrorAction SilentlyContinue
if ($existing) {
...
$pkg = Get-AppxPackage -Name "AIStatusTray.CmdPalExtension"
if ($pkg -and $pkg.Status -eq "Ok") {
    ...
    Write-Host ""
    Write-Host "Open PowerToys Command Palette to see the AI Status dock band." -ForegroundColor Cyan
} else {
